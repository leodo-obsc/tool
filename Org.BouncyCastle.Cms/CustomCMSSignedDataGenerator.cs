	...
	public class CustomCMSSignedDataGenerator : CmsSignedGenerator
	{
		private class SignerInf
		{
			...
			internal SignerInfo ToSignerInfo(DerObjectIdentifier contentType, CmsProcessable content, SecureRandom random)
			{
				...
				ISigner signer = ((outer._signerProvider != null) ? outer._signerProvider : Helper.GetSignatureInstance(algorithm));
				byte[] array = null;
				...
				signer.Init(forSigning: true, new ParametersWithRandom(key, random));
				Stream stream = new BufferedStream(new SigOutputStream(signer));
				...
				stream.Close();
				byte[] array2 = signer.GenerateSignature();
