	...
	public class CertificateHandle
	{
		...
		public static byte[] GetOcspResponse(List<X509Certificate2> certChain, string ocspUrl = null, DateTime? timeCheck = null)
		{
			...
			try
			{
				...
				BigInteger serialNumber = x509CertificateParser.ReadCertificate(certChain[0].GetRawCertData()).SerialNumber;
				OcspReq ocspReq = OcspClient.GenerateOcspRequest(x509CertificateParser.ReadCertificate(certChain[1].GetRawCertData()), serialNumber);
