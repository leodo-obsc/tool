...
namespace SignService.Common.HashSignature.Xml
{
	public class XmlHashSigner : BaseHashSigner, IHashSigner
	{
		...
		private void Init()
		{
			...
			if (_unsignData[0] == 60 && _unsignData[1] == 63 && _unsignData[2] == 120 && _unsignData[3] == 109 && _unsignData[4] == 108 && _unsignData[5] == 32 && _unsignData[6] == 118 && _unsignData[7] == 101 && _unsignData[8] == 114 && _unsignData[9] == 115 && _unsignData[10] == 105 && _unsignData[11] == 111 && _unsignData[12] == 110 && _unsignData[13] == 61 && _unsignData[14] == 34 && _unsignData[15] == 49 && _unsignData[16] == 46 && _unsignData[17] == 49 && _unsignData[18] == 34)
			{
				_versionXML11 = true;
				_unsignData[17] = 48;
			}
		...
		public byte[] GetSecondHashBytes()
		{
			try
			{
				...
				if (_referencesId.Count == 0)
				{
					...
					signature = DsigSignature.CreateSignature(_hashAlgorithm, _addSigningTime ? _signingTime : DateTime.MinValue, text2, "", subjectDN, Convert.ToBase64String(_signerCert), text, _signId, _referenceId, _signTimeId);
				}
				else
				{
					...
					signature = DsigSignature.CreateSignature(_hashAlgorithm, _addSigningTime ? _signingTime : DateTime.MinValue, list, "", subjectDN, Convert.ToBase64String(_signerCert), text, list2, _signId, _signTimeId);
				}
