...
{  
    public class XmlHashSignerV2 : BaseHashSigner, IHashSigner
    {
    	...
    	private static System.Security.Cryptography.Xml.XmlDsigXPathTransform CreateXPathTransform(string XPathString)
    	{
    		XmlElement xmlElement = new XmlDocument().CreateElement("XPath");
    		xmlElement.InnerText = XPathString;
            System.Security.Cryptography.Xml.XmlDsigXPathTransform xmlDsigXPathTransform = new System.Security.Cryptography.Xml.XmlDsigXPathTransform();
    	...
    	public byte[] GetSecondHashBytes()
    	{
    		...
    		if (_referenceIds != null)
    		{
    			foreach (string referenceId in _referenceIds)
    			{
                    ...
                    System.Security.Cryptography.Xml.XmlDsigEnvelopedSignatureTransform transform = new XmlDsigEnvelopedSignatureTransform();
